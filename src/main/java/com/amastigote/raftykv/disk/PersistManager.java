...
import static com.amastigote.raftykv.conf.NodeGlobalConf.KEY_CHECKPOINT_CACHE_SIZE;
import static com.amastigote.raftykv.conf.NodeGlobalConf.KEY_PERSIST_DIR;
...
public final class PersistManager {
    ...
    public void truncateLogEntry(int toIdxInclusive) {
        try {
            stateSerializer.truncateLogEntry(toIdxInclusive);
        } catch (Exception e) {
            log.warn("failed to truncate log entry to idx inclusive {}", toIdxInclusive, e);
        }
    }
}
